...
const csvUrl =
  "https://gist.githubusercontent.com/matovu-ronald/8340bdd802923c9fdb4493d337feeb5e/raw/665e9087442c03ef64e0f052fe722faeff68af43/water_quality_data.csv";
...
d3.csv(csvUrl, rowFormatter).then((data) => {
  ...
  const sulphateGroupedByDistrict = d3.rollups(
    data,
    (v) => d3.mean(v, (d) => d.sulphate),
    (d) => d.district
  );

  const magneisumHardnessGroupedByDistrict = d3.rollups(
    data,
    (v) => d3.mean(v, (d) => d.magnesium_hardness),
    (d) => d.district
  );
  ...
  const barChartUpdateFunction = barChart(
    sulphateGroupedByDistrict,
    "#barchart"
  );
  ...
  const bubbleChartUpdateFunction = bubbleChart(
    magneisumHardnessGroupedByDistrict,
    "#bubblechart"
  );
  ...
  function filterData(selectedRegion, selectedDistrict) {
    ...
    const filteredMagneisumHardnessGroupedByDistrict = d3.rollups(
      filteredData,
      (v) => d3.mean(v, (d) => d.magnesium_hardness),
      (d) => d.district
    );

    const filteredSulphateGroupedByDistrict = d3.rollups(
      filteredData,
      (v) => d3.mean(v, (d) => d.sulphate),
      (d) => d.district
    );

    barChartUpdateFunction(filteredSulphateGroupedByDistrict);
    scatterPlotUpdateFunction(filteredData);
    donutChartUpdateFunction(filteredDataGroupedByDistrict);
    bubbleChartUpdateFunction(filteredMagneisumHardnessGroupedByDistrict);
...
function groupedBarChart(data, selector) {
  ...
  const y = d3
    .scaleLinear()
    .domain([0, d3.max(data, (d) => d.chloride)])
    .nice()
  ...
  svg
    ...
    .attr("y", (d) => y(d.total_hardness))
    .attr("width", x.bandwidth())
    .attr("height", (d) => y(0) - y(d.total_hardness))
  ...
  function update(newData) {
    ...
    y.domain([0, d3.max(newData, (d) => d.total_hardness)]);
